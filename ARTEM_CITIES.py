...
print(start)
def check_the_last_symbol(slovo='абакан'):
    Eend_symbol = (slovo[-1::])  # берет последний символ из города, названного Системой
    if Eend_symbol == 'ь' or Eend_symbol == 'ъ' or Eend_symbol == 'ы' or Eend_symbol == 'ё' or Eend_symbol == 'й':
        Eend_symbol = (slovo[-2:-1:])
    return Eend_symbol

last_symbol = check_the_last_symbol(start)
print(f'назовитее город на букву {last_symbol}')

inp = str(input())  # вводим город
def check_city_in_list(input_city = 'абакан'):
    """проверяет город в списке городов возвращает True"""
    while inp not in all_cities:
        """Программа требует ввести город, пока он не найдется в списке городов"""
        print(f'Нужно ввести название города! на букву {Eend_symbol}')
        inp = str(input())
    return inp

end_symbol = check_the_last_symbol(check_city_in_list()) # последняя буква введенного слова
